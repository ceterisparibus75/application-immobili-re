...
const validLeaseInput = { lotId: VALID_CUID, tenantId: VALID_CUID2, leaseType: "COMMERCIAL_369" as const, startDate: "2024-01-01", durationMonths: 108, baseRentHT: 1500, depositAmount: 3000, paymentFrequency: "MENSUEL" as const, vatApplicable: true, vatRate: 20 }
describe("createLease", () => {
  ...
  it("erreur si lot introuvable", async () => {
    ...
    prismaMock.tenant.findFirst.mockResolvedValue(buildTenantPhysique({ id: VALID_CUID2 }) as unknown)
  ...
  it("erreur si locataire introuvable", async () => {
    mockAuthSession(UserRole.GESTIONNAIRE)
    prismaMock.lot.findFirst.mockResolvedValue({ id: VALID_CUID, buildingId: "b1" } as unknown)
  ...
  it("erreur si lot a deja un bail actif", async () => {
    ...
    prismaMock.lot.findFirst.mockResolvedValue({ id: VALID_CUID, buildingId: "b1" } as unknown)
    prismaMock.tenant.findFirst.mockResolvedValue(buildTenantPhysique({ id: VALID_CUID2 }) as unknown)
    prismaMock.lease.findFirst.mockResolvedValue({ id: "existing", status: "EN_COURS" } as unknown)
    const r = await createLease("society-1", validLeaseInput)
  ...
  it("cree un bail avec succes", async () => {
    ...
    prismaMock.lease.create.mockResolvedValue({ id: "lease-new" } as unknown)
    prismaMock.lot.update.mockResolvedValue({} as unknown)
    const r = await createLease("society-1", validLeaseInput)
...
describe("updateLease", () => {
  ...
  it("resiliation met le lot en VACANT", async () => {
    mockAuthSession(UserRole.GESTIONNAIRE)
    prismaMock.lease.findFirst.mockResolvedValue({ id: VALID_CUID, societyId: "society-1", lotId: "lot-1", status: "EN_COURS" } as unknown)
    prismaMock.lot.update.mockResolvedValue({} as unknown)
    prismaMock.lease.update.mockResolvedValue({} as unknown)
    const r = await updateLease("society-1", { id: VALID_CUID, status: "RESILIE" })
  ...
  it("ne touche pas le lot si deja resilie", async () => {
    mockAuthSession(UserRole.GESTIONNAIRE)
    prismaMock.lease.findFirst.mockResolvedValue({ id: VALID_CUID, societyId: "society-1", lotId: "lot-1", status: "RESILIE" } as unknown)
    prismaMock.lease.update.mockResolvedValue({} as unknown)
    const r = await updateLease("society-1", { id: VALID_CUID, status: "RESILIE" })
    expect(r.success).toBe(true)
...
describe("deleteLease", () => {
  ...
  it("erreur si bail en cours", async () => {
    mockAuthSession(UserRole.ADMIN_SOCIETE)
    prismaMock.lease.findFirst.mockResolvedValue({ id: VALID_CUID, status: "EN_COURS", lotId: "lot-1" } as unknown)
    const r = await deleteLease("society-1", VALID_CUID)
    expect(r.success).toBe(false)
  ...
  it("supprime un bail resilie avec succes", async () => {
    mockAuthSession(UserRole.ADMIN_SOCIETE)
    prismaMock.lease.findFirst.mockResolvedValue({ id: VALID_CUID, status: "RESILIE", lotId: "lot-1" } as unknown)
    prismaMock.lease.delete.mockResolvedValue({} as unknown)

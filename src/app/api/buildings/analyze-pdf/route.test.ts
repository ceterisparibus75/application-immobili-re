...
vi.mock("@/lib/prisma", () => ({ prisma: { building: { findMany: vi.fn() }, lot: { findMany: vi.fn() } } }));

import { POST } from "./route";

describe("POST /api/buildings/analyze-pdf", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("échoue proprement si l'analyse IA n'est pas configurée", async () => {
    const response = await POST(
      new Request("http://localhost/api/buildings/analyze-pdf", { method: "POST" }) as never
    );

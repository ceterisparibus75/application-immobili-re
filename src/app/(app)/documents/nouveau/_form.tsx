...
import { ArrowLeft, Loader2, FileUp, CheckCircle2, X } from "lucide-react";
...
export function UploadDocumentForm({ buildings, lots, leases, tenants }: Props) {
  ...
  // Filtrer les lots par immeuble si un immeuble est sélectionné via lot
  const filteredLots = buildingId
    ? lots.filter((l) => l.buildingId === buildingId)
    : lots;
  ...
  async function handleSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    if (entityType === "building" && buildingId) formData.append("buildingId", buildingId);
    if (entityType === "lot" && lotId) formData.append("lotId", lotId);
    if (entityType === "lease" && leaseId) formData.append("leaseId", leaseId);
    if (entityType === "tenant" && tenantId) formData.append("tenantId", tenantId);
          ...
          <CardContent>
            {!file ? (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground mt-1">PDF, image, Word — max 20 Mo</p>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setBuildingId(""); setLotId(""); setLeaseId(""); setTenantId("");
                }}
            ...
            {entityType === "lot" && (
              <>
                <div className="space-y-2">
                  <Label htmlFor="buildingFilter">Immeuble (filtre)</Label>
                  <select id="buildingFilter" value={buildingId} onChange={(e) => { setBuildingId(e.target.value); setLotId(""); }}
                    className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Pour les diagnostics, assurances, contrats à renouveler…
              </p>
            </div>
          </CardContent>
        </Card>

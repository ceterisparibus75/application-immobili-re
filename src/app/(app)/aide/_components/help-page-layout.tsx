...
const ALL_GUIDES: NavItem[] = [
  ...
  { slug: "glossaire", title: "Glossaire immobilier" },
  { slug: "depannage", title: "Problèmes fréquents et solutions" },
];
  ...
  return (
    <div className="min-h-screen bg-background">
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Table des matières (client — scanne le DOM après hydratation) */}
        <TocClient />

        {/* Contenu */}
        <div className="prose-custom space-y-12">{children}</div>
        ...
        <div className="flex items-center justify-between mt-12 pt-8 border-t">
          ...
          {next ? (
            <Link
              href={`/aide/${next.slug}`}
              className="flex items-center gap-2 text-sm text-right hover:text-primary transition-colors"
            >
              <div>
...
type StepProps = { number: number; title: string; children: React.ReactNode };
...
type InfoBoxProps = { type: "info" | "tip" | "warning"; children: React.ReactNode };

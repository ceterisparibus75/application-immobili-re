          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-wrap gap-1">
              <Link href={buildUrl(year, null)}>
                <Button variant={!buildingId ? "default" : "outline"} size="sm">
                  Tous
                </Button>
              </Link>
              {buildings.map((b) => (
                <Link key={b.id} href={buildUrl(year, b.id)}>
                  <Button
                    variant={buildingId === b.id ? "default" : "outline"}
                    size="sm"
                  >
                    {b.name}
                  </Button>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y">
                      {data.buildings.map((row) => {
                        ...
                        return (
                          <tr key={row.buildingId} className="hover:bg-muted/30 transition-colors">
                            <td className="py-3 pr-4">
                              <p className="font-medium">{row.buildingName}</p>
                              <p className="text-xs text-muted-foreground">{row.buildingCity}</p>
                            </td>
                            <td className="py-3 text-right tabular-nums">
                              {formatCurrency(row.totalProvisions)}
                            </td>
                            <td className="py-3 text-right tabular-nums">
                              {formatCurrency(row.actualCharges)}
                            </td>
                            <td className="py-3 text-right tabular-nums">

  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <div
          className={cn(
            "flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-white text-xs font-bold select-none",
            avatarColor(activeSociety.name)
          )}
        >
          {switching ? "…" : getInitials(activeSociety.name)}
        </div>
      ...
      {open && (
        <div className="absolute top-full left-0 right-0 mt-1 rounded-xl border border-border bg-popover shadow-xl z-50 overflow-hidden">
          <div className="p-1.5">
            ...
            {societies.map((s) => {
              ...
              return (
                ...
                >
                  <div
                    className={cn(
                      "flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-white text-xs font-bold",
                      avatarColor(s.name)
                    )}
                  >
                    {getInitials(s.name)}
                  ...
                  <div className="flex-1 text-left min-w-0">
                    <p className={cn("font-medium truncate leading-tight", isActive && "text-primary")}>
                      {s.name}
                    </p>

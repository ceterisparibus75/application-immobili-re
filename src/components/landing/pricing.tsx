    ...
    <section id="tarifs" className="py-24 sm:py-32 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <div className="inline-flex items-center gap-4 bg-[#F9FAFB] rounded-full px-2 py-2 border border-border/60">
            <button
              onClick={() => setIsYearly(false)}
              className={`px-5 py-2 rounded-full text-sm font-semibold transition-all duration-200 ${
                !isYearly
                  ? "bg-white shadow-sm text-[var(--color-brand-deep)] ring-1 ring-border/60"
                  : "text-muted-foreground hover:text-[var(--color-brand-deep)]"
              }`}
            >
              Mensuel
            </button>
            ...
            >
              Annuel
              <span className="text-xs font-bold text-[var(--color-brand-cyan)] bg-[var(--color-brand-light)] px-2 py-0.5 rounded-full">
                -2 mois
              </span>
            </button>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto items-start">
          {plans.map((plan) => {
            const displayPrice = isYearly
              ...
              : plan.price;
            const savingPct = Math.round((1 - plan.priceYearly / (plan.price * 12)) * 100);
              ...
              >
                ...
                <div className="mb-6">
                  <div className="flex items-baseline gap-1">
                    <span className="text-5xl font-bold text-[var(--color-brand-deep)]">{displayPrice}</span>
                    <span className="text-xl font-semibold text-muted-foreground">&euro;/mois</span>
                  </div>
                  {isYearly ? (
                    <p className="text-xs text-muted-foreground mt-2">
                      {plan.priceYearly}&euro; facturé annuellement{" "}
                      <span className="text-[var(--color-brand-cyan)] font-semibold">(-{savingPct}%)</span>
                    </p>
                  ) : (
                    <p className="text-xs text-muted-foreground mt-2">
                      ou {Math.round(plan.priceYearly / 12)}&euro;/mois en annuel{" "}
                      <span className="text-[var(--color-brand-cyan)] font-semibold">(-{savingPct}%)</span>
                    </p>
                  )}
                </div>
                ...
                <Link
                  href={
                    plan.name === "Institutionnel"
                      ? "/contact"
                      : `/signup?plan=${plan.name.toLowerCase()}&billing=${isYearly ? "yearly" : "monthly"}`
                  }
                  className="block"
                >
                  <Button
